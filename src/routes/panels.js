...
import { Eye, Edit, Trash2, Layout, Calendar, ChevronLeft, ChevronRight, Plus } from "lucide-react";
...
function PanelsPage() {
    ...
    const paginatedPanels = (panels || []).slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
    const handleAction = (id, action) => {
        const params = { panelId: id };
        if (action === "view") {
                    ...
                    <TableCell colSpan={5} className="text-center py-12 text-muted-foreground italic">Nenhum painel salvo encontrado.</TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="icon" title="Visualizar" onClick={() => handleAction(panel.id, "view")}>
                            <Eye className="w-4 h-4"/>
                          </Button>
                          <Button variant="ghost" size="icon" title="Editar" onClick={() => handleAction(panel.id, "edit")}>
                            <Edit className="w-4 h-4"/>
                          </Button>
                          <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive hover:bg-destructive/10" title="Excluir" onClick={() => {
