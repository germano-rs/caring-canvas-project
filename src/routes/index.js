...
import { AlertCircle, MapPin, Calendar, Activity, Info, Columns, Filter, Save } from "lucide-react";
...
function Dashboard() {
    ...
    const [config1, setConfig1] = useState("all");
    ...
    const toStart = (d) => (d ? new Date(`${d}T00:00:00.000Z`).toISOString() : undefined);
    const toEnd = (d) => (d ? new Date(`${d}T23:59:59.999Z`).toISOString() : undefined);
    ...
    const { data: data1, isLoading: isLoading1, error: error1 } = useQuery({
        ...
        queryFn: () => fetchEventsFromDb(config1 === "all" ? undefined : config1, toStart(start1), toEnd(end1), true),
        refetchInterval: 60000,
    });
    const { data: data2 } = useQuery({
        queryKey: ["healthEvents", config1, start2, end2, "compare"],
        queryFn: () => fetchEventsFromDb(config1 === "all" ? undefined : config1, toStart(start2), toEnd(end2), true),
    ...
    const neighborhoodCounts = events.reduce((acc, curr) => {
    ...
    return (<div className="flex-1 overflow-auto p-4 md:p-8 space-y-6">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {!isReadOnly && (<div className="flex flex-wrap items-end gap-2">
          ...
          {(start1 || end1 || start2 || end2) && (<Button variant="ghost" onClick={clearFilters}>Limpar</Button>)}

          <Button variant={isComparisonMode ? "default" : "outline"} onClick={() => setIsComparisonMode(!isComparisonMode)} className="gap-2">
      ...
      {isComparisonMode && !isReadOnly && (<div className="bg-muted/50 p-4 rounded-lg flex flex-wrap items-end gap-4 border border-dashed">
      ...
      {!isComparisonMode && (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card>
      ...
      <div className={`grid grid-cols-1 ${isComparisonMode ? 'lg:grid-cols-2' : ''} gap-6`}>
        ...
        {isComparisonMode && (<Card className="overflow-hidden border-none shadow-lg">
      ...
      {events.length === 0 && !isLoading1 && (<div className="p-8 flex flex-col items-center justify-center space-y-4 bg-muted/20 rounded-xl border-2 border-dashed">
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input id="name" value={panelName} onChange={(e) => setPanelName(e.target.value)} placeholder="Ex: Dengue - Janeiro 2024"/>
            </div>
          </div>
          <DialogFooter>
